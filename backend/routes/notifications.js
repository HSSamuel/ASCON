...
router.get("/my-notifications", verify, async (req, res) => {
  try {
    ...
    const result = notifications.map((n) => {
      ...
      const isRead = doc.isBroadcast
        ? doc.readBy.includes(req.user._id)
        : doc.isRead;

      // ✅ SMART ROUTING FIX:
      // The mobile app requires 'senderId' to route to the Alumni Detail screen.
      // This extracts the ID from the internal 'data' payload generated by notificationHandler.js
      let extractedSenderId = null;
      if (doc.data) {
        extractedSenderId =
          doc.data.id || doc.data.userId || doc.data.senderId || null;
      }

      return {
        ...doc,
        isRead: isRead,
        senderId: extractedSenderId, // Extracted securely so the Flutter app doesn't hit a 404
      };
    });
